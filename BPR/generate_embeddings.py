    ...
    with tqdm(total=len(passage_db)) as pbar:
        while True:
            ...
            if biencoder.hparams.binary:
                ...
                emb = np.where(emb == -1, 0, emb).astype(np.bool)  # convert the embedding data into type of bool(true or false)
                emb = np.packbits(emb).reshape(emb.shape[0], -1) #packbits: pack data in type of unit8
            else:
                emb = emb.cpu().numpy().astype(np.float32)  # raw passage embedding data type

            ids += [passage.id for passage in passages]
            embeddings.append(emb)
            pbar.update(args.batch_size)

    embeddings = np.vstack(embeddings)  # np.vstack: concatenate in dim 0. np.hstack: concatenate in dim 1.
    joblib.dump(dict(ids=ids, embeddings=embeddings), args.output_file)
    #dict(ids=ids, embeddings=embeddings):create a dictionary :
