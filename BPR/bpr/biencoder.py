...
class BiEncoder(LightningModule):
    ...
    def setup(self, step: str) -> None:
        self._train_examples = list(self._load_json_data(self.hparams.train_file))
        rank_zero_info("The number of training examples: %d", len(self._train_examples))
        self._eval_examples = list(self._load_json_data(self.hparams.eval_file))
    ...
    def convert_to_binary_code(self, input_repr: torch.Tensor) -> torch.Tensor:
        if self.training:
            if self.hparams.use_ste:
                hard_input_repr = input_repr.new_ones(input_repr.size()).masked_fill_(input_repr < 0, -1.0)  # input_repr.new_ones(input_repr.size()): create a tensor with the same shape as input_repr, but all elements are 1
    ...
    def _compute_loss(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        scores = torch.matmul(query_repr, passage_repr.transpose(0, 1))
        dense_loss = F.cross_entropy(scores, labels)
    ...
    def add_model_specific_args(parent_parser: ArgumentParser, root_dir: str) -> ArgumentParser:
        ...
        parser.add_argument("--eval_rank_num_hard_negatives", default=30, type=int)
        parser.add_argument("--eval_rank_num_other_negatives", default=30, type=int)
    ...
    def _load_json_data(self, paths: Union[str, list]) -> Iterable[dict]:
    ...
    def _normalize_query(text: str) -> str:
        if text.endswith("?"):  # delete "?" in the end of the question text
            return text[:-1]
        return text
