    ...
    with db.begin(write=True) as txn:
        with open(args.passage_file) as f:
            ...
            for n, row in tqdm(enumerate(tsv_reader)):
                if n == 0:
                    continue
                _id, text, title = int(row[0]), row[1], row[2]   # id is type of int
                json_str = json.dumps((title, text))   #####
                buf.append((str(_id).encode("utf-8"), json_str.encode("utf-8")))  ####binary encoding , why???
                if len(buf) == args.chunk_size:  ####
                    txn.cursor().putmulti(buf)  ####cursor() is used to connect the db
            if buf:
