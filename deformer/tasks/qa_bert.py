...
class BertQa(QaModel):
    def __init__(self, config, **kwargs):
        ...
        if num_classes:
            ...
            self.cls_layer = layers.Dense(config.num_classes,  # binary classifier to generate the answer start logits and end logits
                                          kernel_initializer=initializer,
                                          name='answer_class/dense')

        self.span_layer = layers.Dense(2, kernel_initializer=initializer,
                                       name='answer_classifier/dense')
        self.max_seq_length = config.max_seq_length  # 320
        self.max_answer_span = config.max_answer_span  # 30

        self.attentions = None
        self.encoded_output = None
        self.embeddings = None
        self.logits = None
        if config.use_replace_map:  # True  what meaning?
