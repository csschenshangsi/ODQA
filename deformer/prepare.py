...
def main(args):
    ...
    config.model = args.model  # bert
    config.save_mode = args.save_mode  # both tf and jsonl

    task = args.task
    config.max_seq_length = args.max_seq_length or max_seq_length_map[task]  # 320
    max_first_length = max_first_length_map[task]  # 25, what does it mean of this parameter?
    config.max_first_length = args.max_first_length or max_first_length

    mapped_task = task_map.get(task, task)  # 'qa', setted in __init__.py of dir: tasks
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_file', default='/public6_data/css/data/squad-v1.1/deformer-dev-v1.1.json',
                        help='input dataset file')
    parser.add_argument('-o', '--output_file', default='/public6_data/css/data/squad-v1.1/sbert-examples-dev-v1.1.json',
                        help='output dataset file')
    parser.add_argument('-vf', '--vocab_file', default='/public6_data/css/models/pretrained-bert/bert.vocab',
                        help='vocab file')
    parser.add_argument('-m', '--model', type=str, default='ebert',
    ...
    parser.add_argument('-sm', '--save_mode', default='both',  # tf for training
                        choices=('tf', 'jsonl', 'both'))
    parser.add_argument('-msl', '--max_seq_length', type=int, default=0)  # how to set?
    parser.add_argument('-mfl', '--max_first_length', type=int, default=0)  # how to set?
