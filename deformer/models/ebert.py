...
class Ebert(layers.Layer):
    def __init__(self, config):
        ...
        self.upper_encoder = BertEncoder(config, layer_range=self.upper_range,  ####
                                         name='upper_encoder')
        '''*************************************************************************'''
        # deleted self.attention=None, self.embeddings=None, self.logits=None
    ...
    def call(self, inputs, **kwargs):
        ...
        if embeddings:
            ...
            self.first_embeddings, self.second_embeddings = embeddings  #### only load first_encoded_top, and second_encoded_top
        else:
        ...
        if self.sep_layers > 0:
            ...
            if kwargs.get('fake_cache_first', False):
                ...
                self.first_encoded = self.lower_encoder(lower_inputs, **kwargs)  #### save
            ...
            if kwargs.get('fake_cache_second', False):
                ...
                self.second_encoded = self.lower_encoder(lower_inputs, **kwargs)  #### save
                second_encoded_top = self.second_encoded[-1]   # obtain the k-th layer encoding
        else:
            first_encoded_top = self.first_embeddings
            second_encoded_top = self.second_embeddings
