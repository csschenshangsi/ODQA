...
def evaluate(config):
    ...
    model_class = get_task_model_class(config.model, config.task)  # BertQa or EbertQa, just a class without instantiation
    estimator, model = get_estimator(config, model_class)  # a class BertQa or EbertQa, have instantiated
    record_parser = get_record_parser(config.model, config.task)  # feature_ebert_qa.py -> function record_parser()
    predict_input_fn = input_fn_builder(record_parser, config)  # a function

    output_file = config.output_file
    output_dir = os.path.dirname(output_file)
    tf.io.gfile.makedirs(output_dir)

    example_file = config.ground_truth_file  # dev_examples_files
...
def eval_checkpoint(estimator, predict_input_fn,
    ...
    for batch_result in estimator.predict(predict_input_fn,
                                          ...
                                          yield_single_examples=False):  # has wrong !
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cp', '--checkpoint_path', type=str, default='/public6_data/css/models/ebert-base_s9/squad_v1.1/ebert.ckpt')
